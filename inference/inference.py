...
def predict_fn(inputs, _model):
    ...
    def corr_map(hp_full, m_full, KP_, KPW_, stride):
        num   = F.conv2d(hp_full * m_full, KP_,  stride=stride)
        ...
        pos = y.amax(dim=(2,3))
        neg = (-y).amax(dim=(2,3))
        return torch.where(pos >= neg, pos, -neg).squeeze(0)
    ...
    refine_scales = [max(0.80, s0 - refine_delta), s0, min(1.25, s0 + refine_delta)]
    best_r, _     = pass_once(refine_scales, refine_angles, stride_refine, KP, KPW)
    best_main     = torch.where(best_r.abs() > best_c.abs(), best_r, best_c)
    ...
    for kpp, kpw in zip(KP_PSEUDO, KPW_PSEUDO):
        ...
        if sn is None: sn = s0
        if an is None: an = a0
        refine_scales_n = [max(0.80, sn - refine_delta), sn, min(1.25, sn + refine_delta)]
